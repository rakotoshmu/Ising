...
function X = num2etat(N,m)
    ...
    */
    ...
    X = 2*bitget(m,matrix(1:N^2,N,N))-1;
endfunction
...
function X = ising_exact_N2(J,h,n)
    /*
    ...
    */
    ...
    for m = 1:d
        x = num2etat(m);
        p(m) = pi(x); //probabilité non normalisée
    end
    c = cumsum(p);
    for k = 1:n
        //m suit la loi p
...
function X = ising_MH(J,h,n)
    /*
    Simule le modèle d'Ising selon un algorithme de Metropolis-Hasting, en partant de l'état initial (1,...,1)
    N,J,h les paramètres du modèle d'Ising
...
function Y = ising_gibbs_step(J,h,X,i,j)
    ...
    */
    ...
    Y = X;
    Y(i,j) = 1*(grand(1,1,"def")<p);
endfunction

function X = ising_gibbs_seq(J,h,n)
    /*
    Simule le modèle d'Ising par l'échantilloneur de Gibbs avec balayage séquentiel, en partant de l'état initial (1,...,1)
    N,J,h les paramètres du modèle d'Ising
    n entier le temps de simulation de la chaine
    Renvoie X de taille N x N x n+1 (X(:,:,k) état de la chaîne de Markov à l'instant k+1)
    */
...
function X = ising_gibbs_seq(J,h,n)
    ...
    */
    ...
    for k = 1:n //balayage aléatoire
        X(:,:,k+1) = ising_gibbs_step(J,h,Xtemp,I(1,k),I(2,k));
    end
endfunction
